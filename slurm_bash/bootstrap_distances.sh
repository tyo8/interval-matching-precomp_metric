...
### argument parsing ###
while getopts ":b:o:X:Y:t:n:D:a:v:m:" opt; do
  case $opt in
    b) base_dir=${OPTARG}
    ;;
...
do
		...
		do
			...
			do
				...
				${bsdist_scripter} -x ${barX_fpath} -y ${barY_fpath} -D ${bsdist_homdim} -T ${tagfile} -n ${count} -a ${use_affinity} -v ${verbose} -o ${X_outdir}
			done
		done
	done
done
